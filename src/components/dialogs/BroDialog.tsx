...
interface BroDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Bro['fields']) => Promise<void>;
  defaultValues?: Bro['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function BroDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: BroDialogProps) {
  const [fields, setFields] = useState<Partial<Bro['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'bro');
      await onSubmit(clean as Bro['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Bro['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Edit Bro' : 'New Bro';
