...
export default function BroPage() {
  const [records, setRecords] = useState<Bro[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Bro | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Bro | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Bro | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getBro());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Bro['fields']) {
    await LivingAppsService.createBroEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Bro['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateBroEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Bro"
      subtitle={`${records.length} Bro in the system`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'No results found.' : 'No Bro yet. Add one!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
