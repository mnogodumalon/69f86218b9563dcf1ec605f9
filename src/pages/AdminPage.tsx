...
import { BroViewDialog } from '@/components/dialogs/BroViewDialog';
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('bro');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'bro': return (data as any).bro as Bro[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'bro': return {
        create: (fields: any) => LivingAppsService.createBroEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBroEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBroEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'bro' || dialogState?.entity === 'bro') && (
        <BroDialog
          open={createEntity === 'bro' || dialogState?.entity === 'bro'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'bro' ? handleUpdate : (fields: any) => handleCreate('bro', fields)}
          defaultValues={dialogState?.entity === 'bro' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Bro']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Bro']}
        />
      )}
      {viewState?.entity === 'bro' && (
        <BroViewDialog
          open={viewState?.entity === 'bro'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'bro', record: r }); }}
        />
      )}
