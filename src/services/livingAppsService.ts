...
import type { Bro, CreateBro } from '@/types/app';
...
export class LivingAppsService {
  // --- BRO ---
  static async getBro(): Promise<Bro[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BRO}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Bro[];
    return enrichLookupFields(records, 'bro');
  }
  static async getBroEntry(id: string): Promise<Bro | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BRO}/records/${id}`);
    const record = { record_id: data.id, ...data } as Bro;
    return enrichLookupFields([record], 'bro')[0];
  }
  static async createBroEntry(fields: CreateBro) {
    return callApi('POST', `/apps/${APP_IDS.BRO}/records`, { fields: cleanFieldsForApi(fields as any, 'bro') });
  }
  static async updateBroEntry(id: string, fields: Partial<CreateBro>) {
    return callApi('PATCH', `/apps/${APP_IDS.BRO}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'bro') });
  }
  static async deleteBroEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BRO}/records/${id}`);
  }
